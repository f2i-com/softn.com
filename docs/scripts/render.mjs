...
export function renderBlock(block,doc) {
  switch (block.type) {
    ...
    case 'table': return `<div class="table-scroll${block.columns.length>2?' table-wide':''}" tabindex="0" role="region" aria-label="${e(block.caption)}"><table><caption>${e(block.caption)}</caption><thead><tr>${block.columns.map(col=>`<th scope="col">${inline(col)}</th>`).join('')}</tr></thead><tbody>${block.rows.map(row=>`<tr>${row.map((cell,i)=>`<td data-label="${e(block.columns[i].replace(/`/g,''))}">${inline(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
...
<div class="docs-search" role="search" hidden><label for="docs-search-input">${e(labels.search)}</label><input id="docs-search-input" type="search" placeholder="${e(labels.searchPlaceholder)}" autocomplete="off" aria-controls="search-results"><p id="search-hint" class="search-hint" hidden>${e(labels.searchHint)}</p><div id="search-results" aria-live="polite" data-empty="${e(labels.noResults)}" hidden></div></div>
