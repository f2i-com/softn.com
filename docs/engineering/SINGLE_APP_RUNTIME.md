...
Optional `layout` is `"app"` (the default) or `"page"`. An app pins itself to the viewport and scrolls inside, which suits one that lays out its own screen. A page grows with its content and lets the document scroll, which suits one that reads as a long page: under `"app"` that gets a second scrollbar beside the document's and a header that scrolls away. Only the shell's own layout changes; the bundle is the same either way.

Optional `host` names a same-origin JavaScript module whose default export returns the function the app's `softn.backend.call(action, input, callback)` reaches: `export default async ({config}) => async (action, input) => result`. It is for an app whose actions need something only its host should hold, such as a key, a live connection or a person's consent, and that asks for them by name instead of holding them. The shell loads it once, after the bundle and before the app mounts, and fails closed: a module that is named but will not load, exports no function, or produces no function stops the page with the ordinary failure screen rather than running the app without its backend. The module runs with the page's own authority, outside the bundle's sandbox, so it follows the bundle's location rules and belongs to deployment configuration only. The directory never sets it, because the directory builds each configuration from a fixed set of server-chosen fields and a publisher cannot reach this one. An embedding that renders `SingleApp` itself can pass a `backendCall` prop instead, and that takes precedence.
