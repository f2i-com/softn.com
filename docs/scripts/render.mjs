...
<header class="site-header"><div><a class="wordmark" href="${e(doc.site.homeHref)}">${MARK}${e(doc.site.wordmark??doc.site.name)}</a><nav class="product-links" aria-label="SoftN">${(doc.site.products??[]).map(p=>`<a href="${e(p.href)}">${e(p.label)}</a>`).join('')}<a class="docs-label" href="${e(doc.site.basePath)}" aria-current="page">${e(labels.docsShort??labels.docs)}</a></nav></div><div><button class="theme-toggle" type="button" hidden aria-label="Switch theme" data-to-light="Switch to light mode" data-to-dark="Switch to dark mode">${THEME_ICONS}</button><a class="source-link" href="${e(doc.site.repositoryHref)}" rel="noopener noreferrer">GitHub</a></div></header>
