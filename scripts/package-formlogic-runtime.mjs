 ...
 *                      runtime-manifest.json over every file).
...
const protocols = { nativeProtocol: hostProtocol.nativeProtocol, recordEvents: hostProtocol.recordEvents, editorBridge: Number(bridge[1]) };
...
/** FormLogic's runtime-manifest.json: every file under `prefix` except itself, digested. */
const runtimeManifest = (prefix) => {
  const files = {};
...
try {
  ...
  add('hosted-runtime/runtime-manifest.json', Buffer.from(runtimeManifest('hosted-runtime')));
